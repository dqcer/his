...
public class PageManager {
	 ...
	 * @throws Exception
	 */
	public static Paging initPaging(int pageSize, int currentPageNo, int totalSize) throws Exception {
		Paging paging = new Paging();
		// set pageSize
		if (pageSize > Paging.MAX_PAGE_SIZE) {
			String msg = "Create paging failed! System limit MAX_PAGE_SIZE is " + Paging.MAX_PAGE_SIZE
					+ " , but your setter caused pageSize is " + pageSize
					+ ". You must let pageSize less than MAX_PAGE_SIZE.";
		...
		//log.debug("total size : " + paging.getTotalSize());
		// count pageCount , Set pageCount
		int pageCount = (paging.getTotalSize() - 1) / paging.getPageSize() + 1;
		paging.setTotalPageCount(pageCount);

		// debug currentPageNo , Set currentPageNo
		currentPageNo = currentPageNo < 1 ? (currentPageNo == -1 ? pageCount : 1)
				: (currentPageNo > pageCount ? pageCount : currentPageNo);
		paging.setCurrentPageNo(currentPageNo);

		// Count current page start index in ResultSet.
		// 2015-11-6 10:26:03 修改 以满足DB2分页时使用的between语句
		int start = (paging.getCurrentPageNo() - 1) * paging.getPageSize() + 1;

		// Count current page end index in ResultSet.
		int end = Math.min(paging.getCurrentPageNo() * paging.getPageSize(), paging.getTotalSize());
		paging.setStart(start);
		paging.setEnd(end);
		// Count current page record size and set it in paging.
		paging.setSize(end - start + 1);
